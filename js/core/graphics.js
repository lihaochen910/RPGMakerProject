 ...
 */
Graphics.render = function ( stage, renderTexture = null, renderTarget = null ) {
	if ( this._skipCount === 0 ) {
		var startTime = Date.now ();
		if ( stage ) {
			if ( renderTexture ) {
				this._renderer.bindRenderTexture ( renderTexture );
				this._renderer.render ( stage, renderTexture );
			} else {
				this._renderer.render ( stage );
			}
			if ( renderTarget ) {
				this._renderer.bindRenderTarget ( renderTarget );
			}
...
Graphics._createRenderer = function () {
	...
	var options = { view: this._canvas, antialias: true, forceFXAA: false, resolution: 1, clearBeforeRender: false, preserveDrawingBuffer: false, roundPixels: false };
	try {
...
Graphics.dumpRenderTexture = function ( renderTexture ) {
	// let canvas = this._renderer.extract.canvas ( renderTexture );
	// window.open ( canvas.toDataURL ( 'image/png' ) );

	this._renderer.extract.canvas ( renderTexture ).toBlob ( function ( b ) {
		let a = document.createElement ( 'a' );
		document.body.append ( a );
		a.download = 'dumped_rendertexture.png';
